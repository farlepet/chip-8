...
void chip8::cycle()
{
	...
	switch(opcode & 0xF000)
	{
		case 0x0000:
			switch(opcode & 0x00FF)
			{
				case 0x00E0: //0x00E0: clear screen
					memset(gfx, 0, 32*64); drawFlag = true; break;
		...
		case 0x3000: //0x3XNN: skip next op if V[X] == NN
			if(V[(opcode & 0x0F00) >> 8] == (opcode & 0x00FF)){ pc += 4; break; } pc += 2; break;

		case 0x4000: //0x4XNN: skip next op if V[X] != NN
			if(V[(opcode & 0x0F00) >> 8] != (opcode & 0x00FF)){ pc += 4; break; } pc += 2; break;

		case 0x5000: //0x5XY0: skip next op if V[X] == V[Y]
			if(V[(opcode & 0x0F00) >> 8] == V[(opcode & 0x00F0) >> 4]){ pc += 4; break; } pc += 2; break;
		...
		case 0xE000:
			switch(opcode & 0x00FF)
			{
				case 0x009E: //0xEX9E: Skips next op if key in V[X] is pressed
					if(key[V[(opcode & 0x0F00) >> 8]] != 0){ pc += 4; break; }
					pc += 2; break;
